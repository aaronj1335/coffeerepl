...
# allow global access to the repl for things like command line history
global._repl = repl

readFile join(process.env.HOME, '.coffeerc'), 'utf8', (err, data)->
  if not err
    try
      _ = global._
      returnValue = CoffeeScript.eval "_=(#{data}\n)", {
        filename: 'repl'
        modulename: 'repl'
      }
      if returnValue is undefined
